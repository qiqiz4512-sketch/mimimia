...
| macOS 26.4 | Safari | 26.4 | WebGL 2 | 均衡 | 交互已完成 | 视觉待复验 |
| Windows Server 2025 | Google Chrome 稳定版 | 149.0.7827.201 | WebGL 2 | 兼容 | 已完成 | 通过 |
...
Safari 26.4 在 GitHub 提供的隔离 macOS 26.4 环境中通过原生 SafariDriver 完成全部交互，没有修改用户本机 Safari 的安全设置；但首轮完成截图没有捕捉到角色和灵猫，因此没有把该项标为视觉通过。现已为 Safari 增加稳定 WebGL 画面路径和截图亮度门槛，本地 WebKit 完成画面的高亮像素占比为 14.74%，角色与灵猫清晰可见；仍需把修正推送后由真实 Safari 重跑。首轮记录与未通过的画面保留在 [Safari 数据](evidence/mimimia-safari-26.4-full-flow.json) 和 [Safari 首轮完成画面](evidence/mimimia-safari-26.4-complete.png)，不作为最终通过证据。

Windows Chrome 与 Edge 均使用测试机已安装的稳定版浏览器和真实鼠标事件执行。托管机只有约 4–5 帧，短暂状态可能在一次自动操作结束前已经推进，因此测试用页面状态变化记录确认“蓄力、消散、召唤”确实发生，没有放宽最终结果。两项原始记录见 [Windows 浏览器数据](evidence/mimimia-windows-browser-matrix.json)，完整工作流见 [GitHub Actions 运行记录](https://github.com/lileee/mimimia/actions/runs/29297272868)。
...
macOS Chrome、桌面 Firefox、Windows Chrome 与 Windows Edge 已通过；macOS Safari 的真实交互通过，但修正后的可见画面仍待远程复验。强制 WebGL 2 回退流程已经完整通过。
